...
function MatchForm() {
    ...
    useEffect(() => {
        const fetchPendingBOS = async () => {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/pending-bos`);
            const data = await response.json();
            setPendingBOS(data);
        };
        fetchPendingBOS();
    ...
    return (
        <><Header isLogged={isLogged} user={user} setIsLogged={setIsLogged} setUser={setUser} />
            ...
            <div className="pendingBOSSection">
                <h3>BO en attente de validation</h3>
                {pendingBOS.length > 0 ? (
                    <ul className="pendingBOSList">
                        {pendingBOS.map(bo => (
                            <li key={bo.id_game} className="pendingBOItem">
                                <div className="boTeams">
                                    <span>{bo.team1_name}</span>
                                    <span>vs</span>
                                    <span>{bo.team2_name}</span>
                                    <span className="boTag">BO{bo.bo}</span>
                                </div>
                                <div className="boActions">
                                    <button
                                        onClick={() => declareWinner(bo.id_game, bo.team1_id)}
                                        className="winnerButton"
                                    >
                                        {bo.team1_name} gagne
                                    </button>
                                    <button
                                        onClick={() => declareWinner(bo.id_game, bo.team2_id)}
                                        className="winnerButton"
                                    >
                                        {bo.team2_name} gagne
                                    </button>
                    ...
                    <p className="noPendingBOS">Aucun BO en attente.</p>
                )}
            </div>
        </>
    );
